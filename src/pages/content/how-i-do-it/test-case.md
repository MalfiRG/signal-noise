...
| W.QP.JOBS | Query parameters for the jobs endpoint |
...
**Good:** "HTTP 201. GET response: `mode=Incremental`, `scope.includeSystem=true`, `scope.customPaths` contains `/data`."
...
**The boundary condition template** — for every field that accepts a range of values: minimum valid value, maximum valid value, one below minimum, one above maximum, zero, negative, type mismatch. Applied systematically to schedule fields (hours 0-23, minutes 0-59), pagination parameters, and filter values.
...
|---------------|---------------------|
| POST with `mode=Incremental` + valid scope → 201 | POST with `mode=Incremental` WITHOUT scope → 400 |
| POST with valid resource group ID → 201 | POST with nonexistent resource group ID → 400 |
| PUT to update description → 200 | PUT to change immutable type field → 400 |
...
This is a design decision, not an afterthought. When I write "Precondition: Existing policy with running job," I'm thinking about the test fixture that will create that state. "Precondition: Object storage repository" maps to an infrastructure discovery fixture that skips the test if the lab doesn't have one.
