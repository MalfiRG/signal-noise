...
`.hero-stamp-entrance` uses `animation-fill-mode: both` (not `forwards`). The `backwards` component projects the 0% keyframe values (`filter: blur(12px) brightness(2)`) into the 2.2s `animationDelay` period. Without it, PROVE IT renders with default styles during the delay - fully readable text that then snaps to the blurred 0% state. The 0% keyframe deliberately omits `letter-spacing` - only `filter` and `transform` are projected into the delay. `transform: scale()` is visual-only (no layout impact), but `letter-spacing` is a layout property that would physically widen text past container edges on narrow viewports. The letter-spacing expansion starts at the 30% keyframe when the animation is already playing.
