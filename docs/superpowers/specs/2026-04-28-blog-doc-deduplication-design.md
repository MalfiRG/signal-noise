...
- Memory deletion scope expanded from 2 → **7 files** (adversarial review surfaced 5 untouched stale files; `project-owner.md` additionally violates the workspace-global Veeam-obfuscation rule).
...
| `technical-blog/memory/project-owner.md` | 37 | ~260 | outer | Operator profile; **violates workspace-global Veeam-obfuscation rule** (line 4: "Role: QA Engineer at Veeam") in addition to being stale |
...
- Line 4: *"Role: QA Engineer at Veeam"* — directly violates `MetaOrchestrator/CLAUDE.md §2`: *"Veeam obfuscation: NO Veeam name in ANY output (global scrub rule, all sensitivity tiers)"*. Same axiom as `~/.claude/rules/audit-artifact-scrubbing.md` "Enforcer Artifacts" extension applies at the inventory surface — a checked-in artifact naming the prior employer is a leak whether the file is read by humans or agents.
...
| `technical-blog/memory/project-owner.md` | 37 | **Violates Veeam-obfuscation rule** (`TechnicalBlog/CLAUDE.md §2`) at line 4 PLUS is stale; either justification alone supports deletion |
...
Delete all 7 files outright via `git rm`. NO migration of unique content (every paragraph either contains false claims about the current codebase OR violates the Veeam-obfuscation rule; nothing salvageable).
